...
const UploadForm = ({
  ...
  function handleFileInputChangeEvent(
  ...
  async function uploadFiles(
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_AUTH0_URL}/uploads/`,
        formData,
      );
      return response;
    } catch (error) {
  ...
  async function handleSubmitForm(e: React.MouseEvent<HTMLButtonElement>) {
    try {
      ...
      if (oldFile && newFile && user?.sub) {
        ...
        const response1 = await uploadFiles(
          oldFile,
          newFile,
          numMsgs,
          user.sub,
        ...
        if (response1?.data) {
          const response2 = await axios.get(
            `${import.meta.env.VITE_AUTH0_URL}/msgs/${user.sub}`,
          );
          handleUpdateMessages(JSON.parse(response2.data.msgs));
        }
      }
    } catch (e) {
  ...
  return (
    <div className="upload-form">
      <UploadFile
        fileName="oldFile"
        handleFileInputChangeEvent={handleFileInputChangeEvent}
      />
      <UploadFile
        fileName="newFile"
        handleFileInputChangeEvent={handleFileInputChangeEvent}
      ...
      <NumberInputComponent
        numMsgs={numMsgs}
        handleNumberInputChangeEvent={handleNumberInputChangeEvent}
      />
      <Button
        submit-button
        colorPalette="teal"
